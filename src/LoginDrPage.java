...
public class LoginDrPage extends JFrame {
...
label_login=new JLabel("Doctor Login");
...
log_but.addActionListener(new ActionListener(){
 public void actionPerformed(ActionEvent e){ 
   ...
   if(email.trim().isEmpty() || password.isEmpty()){
       JOptionPane.showMessageDialog(LoginDrPage.this,"Please enter both email and password!");
       return;
   }
   
   // SQL query to get doctor's hashed password and salt
   String query = "SELECT email, password, password_salt FROM doctor WHERE email = ?";
   PreparedStatement pstmt = conn.prepareStatement(query);
   pstmt.setString(1, email);
   ResultSet rs = pstmt.executeQuery();
   
   // If doctor found, verify password
   if(rs.next()){
       ...
       if(storedSalt == null || storedSalt.isEmpty()){
           ...
           if(loginSuccessful){
               ...
               String updateQuery = "UPDATE doctor SET password = ?, password_salt = ? WHERE email = ?";
               ...
               System.out.println("Password upgraded to hashed version for doctor: " + email);
           }
       } else {
       ...
       if(loginSuccessful){
           // Hide login page and go to show doctor appointment page
           setVisible(false);
           new ShowDrAppointmentPage(email).setVisible(true);
       } else {
           JOptionPane.showMessageDialog(LoginDrPage.this,"Wrong email or password!");
       }
   }
   else{
       ...
       JOptionPane.showMessageDialog(LoginDrPage.this,"Login failed: " + ex.getMessage());
   }
 }
});
}
